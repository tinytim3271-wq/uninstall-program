#!/usr/bin/env python3
"""
Reliable clean slate uninstall — Cross-platform program uninstaller.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="Reliable clean slate uninstall",
        description=(
            "Reliable clean slate uninstall removes all traces of a program to allow for a clean reinstall."
        ),
    )
    parser.add_argument(
        "program",
    ...
    parser.add_argument(
        "--license-file",
        help="Override the license file used for runtime verification.",
    )
    parser.add_argument(
        "--public-key-file",
        help="Override the public key file used for license verification.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        licensing.require_valid_license(
            explicit_license_path=args.license_file,
    ...
    print(
        f"\nReliable clean slate uninstall — searching for traces of '{program}' on {SYSTEM}...\n"
    )
